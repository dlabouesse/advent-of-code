...
def launch_probe(target):
    ...
    for y_speed in range(target['min_y'], target['max_x']):
        for x_speed in range(1, target['max_x']+1):
            ...
            position = [0, 0]
            current_max_y = 0
            is_current_velocity_winning = False
            while True:
                ...
                if (position[0] > target['max_x'] or position[1] < target['min_y']):
                    ...
                    if(not is_current_velocity_winning):
                        winning_velocities_nb += 1
                        is_current_velocity_winning = True
    return winning_velocities_nb
